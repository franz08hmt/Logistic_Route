  ...
  return (
    <section className="border-t border-slate-200 px-4 py-4 dark:border-slate-800" aria-labelledby="route-cost-title">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-teal-700 dark:text-teal-400">{t('map.routeCost')}</p>
          <h3 id="route-cost-title" className="mt-1 text-sm font-semibold text-slate-950 dark:text-white">{t('map.routeCost')}</h3>
        </div>
        <button
          type="button"
          className="rounded-lg border border-slate-200 px-2.5 py-2 text-xs font-semibold text-slate-700 transition hover:border-teal-300 hover:text-teal-700 focus-visible:outline-2 focus-visible:outline-teal-600 disabled:opacity-40 dark:border-slate-700 dark:text-slate-300 dark:hover:border-teal-700 dark:hover:text-teal-300"
          onClick={onExport}
      ...
      <div className="mt-3 grid grid-cols-3 gap-2">
        <article className="rounded-lg bg-slate-50 p-2.5 dark:bg-slate-950/60">
          <span className="block text-[10px] text-slate-500">{t('map.totalCost')}</span>
        ...
        <article className="rounded-lg bg-emerald-50 p-2.5 dark:bg-emerald-950/30">
        ...
        <article className="rounded-lg bg-slate-50 p-2.5 dark:bg-slate-950/60">
          <span className="block text-[10px] text-slate-500">{t('map.co2Emissions')}</span>
