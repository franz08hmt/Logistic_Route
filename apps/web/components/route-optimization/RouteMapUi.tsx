...
export function MapStatusOverlays({
  ...
  return (
    <>
      <div className="pointer-events-none absolute bottom-20 right-3 z-[400] rounded-lg border border-white/60 bg-white/90 px-2.5 py-1.5 text-[10px] font-medium text-slate-600 shadow backdrop-blur dark:border-slate-700 dark:bg-slate-950/90 dark:text-slate-300 lg:bottom-3">
        OpenStreetMap · Leaflet · OSRM
      </div>
      {routingState === 'loading' && (
        <p className="absolute right-3 top-40 z-[450] flex items-center gap-2 rounded-lg bg-slate-950/80 px-3 py-2 text-xs font-medium text-white shadow-lg backdrop-blur" role="status">
      ...
      {routingState === 'fallback' && (
        <p className="absolute right-3 top-40 z-[450] max-w-xs rounded-lg border border-amber-200 bg-white/95 px-3 py-2 text-xs text-amber-800 shadow-lg backdrop-blur dark:border-amber-900 dark:bg-slate-950/95 dark:text-amber-300" role="status">
          {t('map.routingFallback')}
        </p>
      )}
      {!hasResult && (
        <div className="pointer-events-none absolute left-1/2 top-1/2 z-[400] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-white/70 bg-white/90 px-6 py-5 text-center shadow-xl backdrop-blur dark:border-slate-700 dark:bg-slate-950/90" role="status">
          <span className="mx-auto grid size-10 place-items-center rounded-full bg-teal-50 text-xl text-teal-700 dark:bg-teal-950 dark:text-teal-300" aria-hidden="true">⌖</span>
          <strong className="mt-3 block text-sm text-slate-950 dark:text-white">{t('map.emptyTitle')}</strong>
          <small className="mt-1 block text-xs text-slate-500">{t('map.emptyDescription')}</small>
        </div>
      )}
    </>
  );
...
export function RouteLegend({
  ...
  return (
    <div className="absolute bottom-3 left-1/2 z-[450] hidden -translate-x-1/2 items-center gap-3 rounded-full border border-white/70 bg-white/90 px-3 py-2 text-[10px] font-medium text-slate-700 shadow-lg backdrop-blur md:flex dark:border-slate-700 dark:bg-slate-950/90 dark:text-slate-200" aria-label={t('map.legend')}>
