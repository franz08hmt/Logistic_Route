    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h2 id="route-list-title" className="mt-1 text-base font-semibold text-slate-950 dark:text-white">
            ...
            {result.routes.map((route, index) => {
              ...
              return (
                <article
                  ...
                  className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950/30"
                >
                  <header className="flex items-center justify-between gap-3 border-b border-slate-200 px-3.5 py-3 dark:border-slate-800">
                    <span>
                      <small className="block text-[10px] font-semibold uppercase tracking-wide text-slate-500">
                      ...
                      <ol
                        ...
                        className={`min-h-24 space-y-2 p-3 transition-colors ${
                          snapshot.isDraggingOver
                            ? 'bg-teal-50 ring-2 ring-inset ring-dashed ring-teal-500 dark:bg-teal-950/30'
                            : 'bg-slate-50/70 dark:bg-slate-950/40'
                        }`}
                      >
                        {route.stops.length === 0 && !snapshot.isDraggingOver && (
                          <li className="grid min-h-16 place-items-center rounded-lg border border-dashed border-slate-300 px-3 text-center text-xs text-slate-500 dark:border-slate-700">
                          ...
                          >
                            {(dragProvided, dragSnapshot) => (
                              <li
                                ...
                                className={`grid grid-cols-[auto_1fr_auto] items-center gap-2.5 rounded-lg border bg-white p-2.5 transition-[border-color,box-shadow,transform] dark:bg-slate-900 ${
                                  dragSnapshot.isDragging
                                    ? 'border-teal-500 shadow-xl ring-2 ring-teal-500/20'
                                    : 'border-slate-200 shadow-sm dark:border-slate-800'
                                }`}
                              >
                                <span className="grid size-6 place-items-center rounded-full bg-teal-600 text-[10px] font-bold text-white">
                                  {stopIndex + 1}
                                </span>
                                ...
                                {onMoveStop && (
                                  <button
                                    ...
                                    className="grid size-9 touch-none place-items-center rounded-lg text-lg text-slate-400 transition-colors hover:bg-slate-100 hover:text-teal-700 focus-visible:outline-2 focus-visible:outline-teal-600 dark:hover:bg-slate-800 dark:hover:text-teal-300"
