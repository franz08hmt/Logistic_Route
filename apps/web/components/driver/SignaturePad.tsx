...
export const SignaturePad = forwardRef<SignaturePadHandle, {
  ...
  return (
    <div>
      <div className="relative overflow-hidden rounded-sm border border-dashed border-slate-300 bg-white dark:border-slate-600">
        <canvas
          ref={canvasRef}
      ...
      <div className="mt-2 flex items-center justify-between gap-3">
        <p className="text-xs leading-5 text-slate-500 dark:text-slate-400">{t('signature.canvasHint')}</p>
        <button
          type="button"
          className="shrink-0 rounded-sm px-3 py-2 text-xs font-semibold text-slate-600 transition hover:bg-slate-100 focus-visible:outline-2 focus-visible:outline-amber-600 disabled:opacity-50 dark:text-slate-300 dark:hover:bg-slate-800"
          onClick={clear}
          disabled={disabled}
        >
          {t('signature.clear')}
