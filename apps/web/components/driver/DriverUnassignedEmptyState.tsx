...
export function DriverUnassignedEmptyState({
  ...
  return (
    <section
      className="overflow-hidden rounded-2xl border border-teal-100 bg-white shadow-sm dark:border-teal-950 dark:bg-slate-900"
      aria-labelledby="driver-onboarding-title"
    >
      <div className="border-b border-slate-100 bg-gradient-to-br from-teal-50 via-white to-slate-50 px-5 py-8 text-center dark:border-slate-800 dark:from-teal-950/40 dark:via-slate-900 dark:to-slate-950 sm:px-10 sm:py-10">
        <div className="mx-auto grid size-16 place-items-center rounded-2xl bg-teal-100 text-teal-700 ring-8 ring-teal-50 dark:bg-teal-900/70 dark:text-teal-300 dark:ring-teal-950/50" aria-hidden="true">
        ...
        <p className="mt-6 text-xs font-bold uppercase tracking-[0.18em] text-teal-700 dark:text-teal-400">{t('driver.onboardingEyebrow')}</p>
      ...
      <div className="px-5 py-7 sm:px-10 sm:py-8">
        ...
        <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
          <button
            type="button"
            className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-teal-600 px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-teal-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-600 disabled:cursor-wait disabled:opacity-70"
            onClick={onRefresh}
...
function Step({
  ...
  const styles = {
    ...
    current: 'border-teal-300 bg-teal-50 text-teal-800 ring-2 ring-teal-600/10 dark:border-teal-800 dark:bg-teal-950/40 dark:text-teal-300',
  ...
  return (
    <li className="relative flex items-center gap-3 md:flex-col md:items-center md:text-center">
      ...
      {state === 'current' && <span className="text-xs text-teal-700 dark:text-teal-400">…</span>}
    </li>
  );
}
