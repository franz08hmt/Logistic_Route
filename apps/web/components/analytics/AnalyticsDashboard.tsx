...
const AnalyticsCharts = dynamic(() => import('./AnalyticsCharts'), {
  ...
  loading: () => (
    <div className="grid gap-5 lg:grid-cols-2" aria-busy="true">
      <div className="h-96 animate-pulse rounded-sm bg-slate-200/70 dark:bg-slate-800 lg:col-span-2" />
...
export function AnalyticsDashboard() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 rounded-sm border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none sm:flex-row sm:items-center sm:justify-between">
        <div className="inline-flex w-fit rounded-sm bg-slate-100 p-1 dark:bg-slate-950" aria-label={t('analytics.periodSelector')}>
          {PERIODS.map((days) => (
            <button
              key={days}
              type="button"
      ...
      {!history && isLoading ? (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label={t('analytics.loading')} aria-busy="true">
          {Array.from({ length: 4 }, (_, index) => (
            <div key={index} className="h-32 animate-pulse rounded-sm bg-slate-200/70 dark:bg-slate-800" />
          ))}
        </div>
      ) : history ? (
        <>
          <div className={`grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4 ${isLoading ? 'opacity-70' : ''}`} aria-live="polite">
            {summaryCards.map((card) => (
              <article key={card.label} className="rounded-sm border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
                ...
                <small className="mt-1 block text-xs text-slate-500 dark:text-slate-400">{card.helper}</small>
              </article>
            ))}
          </div>
          ...
          {history.data_points.length ? (
            <>
              ...
              <section className="overflow-hidden rounded-sm border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
                <div className="border-b border-slate-200 px-5 py-4 dark:border-slate-800">
                  <h2 className="font-bold text-slate-950 dark:text-white text-base uppercase tracking-[0.14em]">{t('analytics.tableTitle')}</h2>
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{t('analytics.tableDescription')}</p>
                </div>
                <div className="overflow-x-auto">
                  <table className="min-w-full text-left text-sm">
                    <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400 dark:bg-slate-950/60">
                      <tr>
            ...
            <section className="rounded-sm border border-dashed border-slate-300 bg-white px-6 py-16 text-center dark:border-slate-700 dark:bg-slate-900">
              <div className="mx-auto grid size-12 place-items-center rounded-full bg-amber-50 text-xl dark:bg-amber-950">↗</div>
              <h2 className="mt-4 font-bold text-slate-950 dark:text-white text-base uppercase tracking-[0.14em]">{t('analytics.emptyTitle')}</h2>
              <p className="mx-auto mt-2 max-w-md text-sm text-slate-500 dark:text-slate-400">{t('analytics.emptyDescription')}</p>
            </section>
          )}
        </>
      ) : null}
