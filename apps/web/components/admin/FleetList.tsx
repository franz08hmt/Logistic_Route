  ...
  if (vehicles.length === 0) {
    return (
      <div className="grid min-h-64 place-items-center px-4 py-12 text-center" role="status">
        <div>
          <span className="mx-auto grid size-11 place-items-center rounded-full bg-teal-50 text-lg text-teal-700 dark:bg-teal-950 dark:text-teal-300" aria-hidden="true">▣</span>
          <h3 className="mt-3 text-sm font-semibold text-slate-950 dark:text-white">{t('fleet.emptyTitle')}</h3>
          <p className="mt-1 text-sm text-slate-500">{t('fleet.emptyDescription')}</p>
        </div>
      </div>
    );
  }
    ...
    <>
      <div className="grid gap-3 p-4 sm:grid-cols-2 md:hidden">
        {vehicles.map((vehicle) => (
          <article key={vehicle.id} className="rounded-xl border border-slate-200 p-4 dark:border-slate-800">
            ...
            {vehicles.map((vehicle) => (
              <tr key={vehicle.id} className="transition hover:bg-slate-50/70 dark:hover:bg-slate-800/40">
                ...
                <td className="px-5 py-4">
                  ...
                  ) : <span className="text-sm text-slate-500">{t('fleet.unassigned')}</span>}
                </td>
