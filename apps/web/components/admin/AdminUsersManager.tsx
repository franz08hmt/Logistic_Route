  ...
  return (
    <section className="space-y-4" aria-labelledby="users-table-heading">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 id="users-table-heading" className="text-lg font-semibold text-slate-950 dark:text-white">{t('admin.users.listTitle')}</h2>
          <p className="mt-1 text-sm text-slate-500">{t('admin.users.total', { count: users.length })}</p>
        </div>
        <button type="button" className="h-10 rounded-xl border border-slate-300 bg-white px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-teal-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800" onClick={() => void loadUsers()}>
          {t('admin.users.refresh')}
        </button>
      </div>

      {error && <p className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700 dark:border-rose-900 dark:bg-rose-950/50 dark:text-rose-300" role="alert">{error}</p>}
      {notice && <p className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/50 dark:text-emerald-300" role="status">{notice}</p>}

      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm shadow-slate-950/5 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
        <table className="min-w-full divide-y divide-slate-200 text-left text-sm dark:divide-slate-800">
          <thead className="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:bg-slate-950/50">
            <tr>
            ...
            {isLoading ? (
              Array.from({ length: 3 }).map((_, index) => (
                <tr key={index} aria-hidden="true">
                  <td className="px-5 py-4" colSpan={5}><span className="block h-8 animate-pulse rounded-lg bg-slate-100 dark:bg-slate-800" /></td>
                </tr>
              ))
            ) : users.length === 0 ? (
              <tr><td className="px-5 py-12 text-center text-slate-500" colSpan={5}>{t('admin.users.empty')}</td></tr>
            ) : users.map((user) => {
              ...
              return (
                <tr key={user.id} className="text-slate-700 dark:text-slate-300">
                  <td className="px-5 py-4">
                    ...
                    <span className="mt-0.5 block text-xs text-slate-500">{user.email}{user.phone_number ? ` · ${user.phone_number}` : ''}</span>
                  </td>
                  ...
                  <td className="whitespace-nowrap px-5 py-4 text-slate-500">{new Intl.DateTimeFormat(locale === 'vi' ? 'vi-VN' : 'en-US', { dateStyle: 'medium' }).format(new Date(user.created_at))}</td>
                  <td className="px-5 py-4 text-right">
                    <div className="flex flex-wrap justify-end gap-2">
                      {user.role === 'DRIVER' && user.status === 'ACTIVE' && !vehicles.some((vehicle) => vehicle.driver_id === user.id) && (
                        <button type="button" className="rounded-lg bg-teal-50 px-3 py-2 text-xs font-semibold text-teal-800 hover:bg-teal-100 focus-visible:outline-2 focus-visible:outline-teal-600 dark:bg-teal-950/50 dark:text-teal-300 dark:hover:bg-teal-950" onClick={() => setSelectedDriver(user)}>
                          {t('admin.assignment.assignVehicle')}
                        </button>
                      )}
                      {currentUser?.role === 'ADMIN' && (
                        <button type="button" className={`rounded-lg px-3 py-2 text-xs font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:opacity-40 ${nextStatus === 'ACTIVE' ? 'bg-teal-600 text-white hover:bg-teal-700 focus-visible:outline-teal-600' : 'border border-rose-200 text-rose-700 hover:bg-rose-50 focus-visible:outline-rose-600 dark:border-rose-900 dark:text-rose-300 dark:hover:bg-rose-950/40'}`} disabled={updatingId === user.id || isSelf} title={isSelf ? t('admin.users.selfActionDisabled') : undefined} onClick={() => void updateStatus(user, nextStatus)}>
