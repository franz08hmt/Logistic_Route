...
export function DriverCard({ driver }: { driver: DriverDetail }) {
  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-slate-300 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          <span className="grid size-11 shrink-0 place-items-center rounded-full bg-teal-50 text-sm font-bold text-teal-700 dark:bg-teal-950 dark:text-teal-300">
      ...
      <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/60">
        {driver.vehicle_id ? (
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-slate-500">{t('drivers.noVehicle')}</p>
            <Link
              className="inline-flex min-h-10 items-center justify-center rounded-xl bg-teal-600 px-4 text-sm font-semibold text-white hover:bg-teal-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-600"
              href="/fleet"
            >
      ...
      <dl className="mt-4 grid grid-cols-3 divide-x divide-slate-200 rounded-xl border border-slate-200 dark:divide-slate-800 dark:border-slate-800">
        <div className="p-3 text-center">
          <dt className="text-[11px] leading-4 text-slate-500">{t('drivers.deliveredToday')}</dt>
          <dd className="mt-1 text-lg font-bold text-emerald-600 dark:text-emerald-400">
            {driver.delivered_today_count}
          </dd>
        </div>
        <div className="p-3 text-center">
          <dt className="text-[11px] leading-4 text-slate-500">{t('drivers.activeOrders')}</dt>
          <dd className="mt-1 text-lg font-bold text-sky-600 dark:text-sky-400">
            {driver.active_orders_count}
          </dd>
        </div>
        <div className="p-3 text-center">
          <dt className="text-[11px] leading-4 text-slate-500">{t('drivers.failedToday')}</dt>
