    ...
    >
      {notification && (
        <div className="max-h-[70vh] overflow-y-auto px-5 py-6 sm:px-6">
          <div className="mx-auto w-full max-w-[20rem] rounded-[2.5rem] border-[8px] border-slate-900 bg-slate-100 p-2 shadow-2xl dark:border-slate-700 dark:bg-slate-950">
            <div className="mx-auto mb-3 h-5 w-24 rounded-b-xl bg-slate-900 dark:bg-slate-700" />
            <div className={`rounded-3xl p-4 ${isZalo ? 'bg-sky-50 dark:bg-sky-950/40' : 'bg-violet-50 dark:bg-violet-950/40'}`}>
              ...
              <div className="mt-5 rounded-2xl rounded-tl-sm bg-white p-4 text-sm shadow-sm dark:bg-slate-900">
                ...
                {trackingUrl && (
                  <a className="mt-3 block break-all font-semibold text-teal-700 underline underline-offset-2 dark:text-teal-300" href={trackingUrl} target="_blank" rel="noreferrer">
                    {t('notifications.openTracking')}
                  </a>
                )}
              </div>
              <p className="mt-3 text-center text-[11px] text-slate-500">{new Date(notification.sent_at).toLocaleString()}</p>
            </div>
          </div>
          <button type="button" className="mx-auto mt-5 flex items-center justify-center rounded-lg bg-teal-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-teal-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-600" onClick={() => void copyMessage()}>
            {copyState === 'copied' ? t('notifications.copied') : t('notifications.copyMessage')}
          </button>
          <p className={`mt-2 text-center text-xs ${copyState === 'error' ? 'text-rose-600' : 'text-slate-500'}`} aria-live="polite">
