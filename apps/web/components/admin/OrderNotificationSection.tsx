  ...
  return (
    <section className="mt-7" aria-labelledby="customer-notifications-title">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 id="customer-notifications-title" className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">{t('notifications.title')}</h3>
          <p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">{t('notifications.description')}</p>
        </div>
      </div>

      {!order.customer_phone ? (
        <p className="mt-3 rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-500 dark:border-slate-700">{t('notifications.noPhone')}</p>
      ) : (
        <div className="mt-3 flex flex-col gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/50 sm:flex-row">
          <label className="flex-1 text-xs font-semibold text-slate-600 dark:text-slate-300">
            {t('notifications.chooseChannel')}
            <select value={channel} onChange={(event) => setChannel(event.target.value as NotificationChannel)} className="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 dark:border-slate-700 dark:bg-slate-900 dark:text-white">
              <option value="ZALO_ZNS">{t('notifications.zalo')}</option>
              <option value="SMS_BRANDNAME">{t('notifications.sms')}</option>
            </select>
          </label>
          <button type="button" disabled={!hasTemplate || isResending} className="self-end rounded-lg bg-teal-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-teal-700 disabled:cursor-not-allowed disabled:opacity-50" onClick={() => void resend()}>
            {isResending ? t('notifications.resending') : t('notifications.resend')}
          </button>
        </div>
      )}
      {!hasTemplate && order.customer_phone && <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">{t('notifications.noTemplate')}</p>}
      ...
      {isLoading ? (
        <p className="mt-4 animate-pulse text-sm text-slate-500">{t('notifications.loading')}</p>
      ) : notifications.length === 0 ? (
        <p className="mt-4 rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-500 dark:border-slate-700">{t('notifications.empty')}</p>
      ) : (
        <ul className="mt-4 space-y-3">
          {notifications.map((notification) => (
            <li key={notification.id} className="rounded-xl border border-slate-200 p-4 dark:border-slate-800">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <span className="text-xs text-slate-500">{new Intl.DateTimeFormat(locale === 'vi' ? 'vi-VN' : 'en-US', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(new Date(notification.sent_at))}</span>
              </div>
              ...
              <div className="mt-3 flex items-center justify-between gap-3">
                ...
                <button type="button" className="text-sm font-semibold text-teal-700 hover:underline dark:text-teal-300" onClick={() => setPreview(notification)}>{t('notifications.preview')}</button>
              </div>
            </li>
          ))}
        </ul>
