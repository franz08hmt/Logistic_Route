    ...
    <section className="space-y-5">
      <div className="overflow-hidden rounded-sm border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <header className="flex flex-col gap-3 border-b border-slate-200 p-5 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="font-bold text-slate-950 dark:text-white text-base uppercase tracking-[0.14em]">{t('dispatch.pendingOrders')}</h2>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{t('dispatch.pendingHint')}</p>
          </div>
        ...
        {error && (
          <p className="m-4 rounded-sm border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700 dark:border-rose-900 dark:bg-rose-950/50 dark:text-rose-300" role="alert">
            {error}
          </p>
        )}
        ...
        {isLoading ? (
          <div className="space-y-3 p-5" aria-busy="true">
            {Array.from({ length: 4 }, (_, index) => (
              <div className="h-20 animate-pulse rounded-sm bg-slate-100 dark:bg-slate-800" key={index} />
            ))}
          </div>
        ) : orders.length === 0 ? (
          <div className="grid min-h-64 place-items-center p-8 text-center">
            <div>
              <span className="mx-auto grid size-12 place-items-center rounded-full bg-amber-50 text-amber-700 dark:bg-amber-950 dark:text-amber-300">✓</span>
              <h3 className="mt-3 font-bold text-slate-950 dark:text-white text-base uppercase tracking-[0.14em]">{t('dispatch.emptyTitle')}</h3>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{t('dispatch.emptyDescription')}</p>
            </div>
          </div>
        ) : (
            ...
            {orders.map((order) => (
              <label
                className={`flex cursor-pointer gap-4 p-4 transition hover:bg-slate-50 dark:hover:bg-slate-800/50 sm:p-5 ${
                  selectedOrderIds.has(order.id) ? 'bg-amber-50/60 dark:bg-amber-950/20' : ''
                }`}
                key={order.id}
              >
                <input
                  className="mt-1 size-4 shrink-0 rounded border-slate-300 accent-amber-600"
                  type="checkbox"
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-1 block text-xs leading-5 text-slate-500 dark:text-slate-400">{order.address}</span>
                </span>
              </label>
            ))}
          </div>
      ...
      <aside className="grid gap-4 md:grid-cols-2">
        <div className="rounded-sm border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
          <span className="text-xs font-semibold uppercase tracking-[0.15em] text-amber-700 dark:text-amber-400">{t('dispatch.planEyebrow')}</span>
          <h2 className="mt-1 font-bold text-slate-950 dark:text-white text-base uppercase tracking-[0.14em]">{t('dispatch.planTitle')}</h2>
          ...
          <dl className="mt-5 grid grid-cols-2 gap-3">
            <div className="rounded-sm bg-slate-50 p-3 dark:bg-slate-950/60">
              <dt className="text-xs text-slate-500 dark:text-slate-400">{t('dispatch.selectedStops')}</dt>
              <dd className="mt-1 text-xl font-bold text-slate-950 dark:text-white">{selectedOrders.length}</dd>
            </div>
            <div className="rounded-sm bg-slate-50 p-3 dark:bg-slate-950/60">
              <dt className="text-xs text-slate-500 dark:text-slate-400">{t('dispatch.totalWeight')}</dt>
              <dd className="mt-1 text-xl font-bold text-slate-950 dark:text-white">{numberFormatter.format(totalWeight)} kg</dd>
            </div>
          </dl>

          {isOverCapacity && (
            <p className="mt-3 rounded-sm bg-orange-50 px-3 py-2 text-xs leading-5 text-orange-800 dark:bg-orange-950/40 dark:text-orange-300" role="alert">
              {t('dispatch.capacityExceeded', { capacity: numberFormatter.format(selectedDriver.capacity_kg) })}
            </p>
          )}
          {selectedOrders.length === 1 && (
            <p className="mt-3 text-xs text-orange-700 dark:text-orange-300">{t('dispatch.minimumStops')}</p>
          )}

          <button
            className="mt-5 inline-flex min-h-11 w-full items-center justify-center rounded-sm bg-amber-700 dark:bg-amber-400 px-4 text-sm font-semibold text-white dark:text-slate-950 hover:bg-amber-800 dark:hover:bg-amber-300 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-600 disabled:cursor-not-allowed disabled:opacity-50"
            type="button"
        ...
        {result && (
          <div className="rounded-sm border border-amber-200 bg-amber-50 p-5 dark:border-amber-900 dark:bg-amber-950/30" role="status">
            <span className="text-xs font-semibold uppercase tracking-wide text-amber-700 dark:text-amber-300">{t('dispatch.success')}</span>
            <h3 className="mt-1 font-bold text-slate-950 dark:text-white text-base uppercase tracking-[0.14em]">{result.driver_name} · {result.license_plate}</h3>
            ...
            <ol className="mt-4 space-y-2">
              {[...result.stops].sort((a, b) => a.stop_sequence - b.stop_sequence).map((stop) => (
                <li className="flex gap-3 text-sm" key={stop.order_id}>
                  <span className="grid size-6 shrink-0 place-items-center rounded-full bg-amber-700 dark:bg-amber-400 text-xs font-bold text-white dark:text-slate-950">{stop.stop_sequence}</span>
                  <span><strong className="block text-slate-900 dark:text-white">{stop.order_code}</strong><small className="text-slate-500 dark:text-slate-400">{stop.address}</small></span>
                </li>
              ))}
            </ol>
          </div>
      ...
      >
        <div className="space-y-4 p-5 sm:p-6">
          <p className="rounded-sm bg-orange-50 p-4 text-sm leading-6 text-orange-900 dark:bg-orange-950/40 dark:text-orange-200">
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <button className="h-10 rounded-sm border border-slate-300 px-4 text-sm font-semibold dark:border-slate-700" type="button" onClick={() => setRegionMismatch(null)}>
              {t('dispatch.chooseAgain')}
            </button>
            <button className="h-10 rounded-sm bg-orange-600 px-4 text-sm font-semibold text-white hover:bg-orange-700" type="button" onClick={() => void executeDispatch(true)}>
