...
const roleStyles = {
  ADMIN: 'bg-orange-50 text-orange-700 dark:bg-orange-950/60 dark:text-orange-300',
  DISPATCHER: 'bg-amber-50 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300',
    ...
    <main className="grid min-h-screen bg-slate-50 dark:bg-slate-950 lg:grid-cols-[minmax(0,1.05fr)_minmax(480px,.95fr)]">
      ...
      <section className="relative flex min-h-screen items-center justify-center px-4 py-12 sm:px-8" aria-label={t('auth.loginRegion')}>
        ...
        <div className="w-full max-w-md">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-amber-700 dark:text-amber-400">{t('auth.secureAccess')}</p>
          ...
          <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
            ...
            {error && (
              <p className="rounded-sm border border-rose-200 bg-rose-50 px-3.5 py-3 text-sm text-rose-700 dark:border-rose-900 dark:bg-rose-950/50 dark:text-rose-300" role="alert">
                {error}
              </p>
            )}

            <button
              className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-sm bg-amber-700 dark:bg-amber-400 px-4 text-sm font-semibold text-white dark:text-slate-950 transition hover:bg-amber-800 dark:hover:bg-amber-300 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-600 disabled:opacity-60"
              type="submit"
          ...
          <div className="mt-8 border-t border-slate-200 pt-6 dark:border-slate-800">
            <div className="mb-3 flex items-center justify-between gap-4">
              ...
              <small className="text-xs text-slate-500 dark:text-slate-400">{t('auth.quickFill')}</small>
            </div>
            <div className="space-y-2">
              {demoAccounts.map((account) => (
                <button
                  key={account.role}
                  type="button"
                  className="flex w-full items-center gap-3 rounded-sm border border-slate-200 bg-white px-3 py-2.5 text-left transition hover:border-amber-300 hover:bg-amber-50/40 focus-visible:outline-2 focus-visible:outline-amber-600 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-amber-800 dark:hover:bg-amber-950/30"
                  onClick={() => {
                ...
                >
                  <span className={`rounded-sm px-2 py-1 text-[10px] font-bold ${roleStyles[account.role]}`}>{account.role}</span>
                  <span className="min-w-0">
                    <strong className="block text-sm text-slate-800 dark:text-slate-200">{t(account.labelKey)}</strong>
                    <small className="block truncate text-xs text-slate-500 dark:text-slate-400">{account.email}</small>
                  </span>
                </button>
              ))}
            </div>
          ...
          <p className="mt-6 text-center text-sm text-slate-600 dark:text-slate-400">
            {t('auth.noAccount')}{' '}
            <Link className="font-semibold text-amber-700 hover:text-amber-800 dark:text-amber-400 dark:hover:text-amber-300" href="/register">
              {t('auth.register')}
            </Link>
          </p>
        </div>
